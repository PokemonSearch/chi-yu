...
async function load()
{
    ...
    while(!(calc_det < lim) || isNaN(calc_det))
    {
        try
        {
            ...
            data_str = fetchHeader+"data/api/"+chosen_pkmn
            var spec_data = await fetch(fetchHeader+"data/api/"+chosen_pkmn+"/species.json").then((response) => response.json());
            console.log("sp data:",spec_data);
            var variants = ["/"]
            var form_names = [""];
            ...
            chiyu_img = loadImage(fetchHeader+"data/sprites/1004/front_default.png");
            pkmn_img = loadImage(fetchHeader+"data/sprites/"+chosen_pkmn+"/"+form_names[chosen_variant]+"/front_default.png");
    ...
    if(def_item != null)
    {
        ...
        item_img = loadImage(fetchHeader+"data/items/sprites/"+internal_name+"/"+internal_name+".png");
    }
